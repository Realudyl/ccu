...
void sPrivDrvPktLogin(void)
{
    ...
    && ((sGetTimestamp() > u32Time) && ((sGetTimestamp() - u32Time) > 10)))
    {
        u32Time = sGetTimestamp();
        EN_SLOGI(TAG, "请求slave端重新签到");
        sPrivDrvSetCmdAA(false, cPrivDrvGunIdChg, ePrivDrvCfgCmdTypeSet, ePrivDrvCmdRstMax, ePrivDrvParamAddr018, 1, &u8Temp, 0);
    }
}
