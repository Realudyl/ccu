...
bool sPrivDrvShellSetCpSn(const stShellPkt_t *pkg)
{
    ...
    bRst = sPrivDrvSetCmdAA(true, (u8)i32GunId, ePrivDrvCfgCmdTypeSet, ePrivDrvCmdRstMax, ePrivDrvParamAddr001, cPrioDrvLenParamSn, u8Sn, 0);
    if(bRst)
    {
        sPrivDrvGetCmd0A(false, &pCmd0A);
        memset(u8Sn, 0, sizeof(u8Sn));
...
bool sPrivDrvShellSetCpName(const stShellPkt_t *pkg)
{
    ...
    bRst = sPrivDrvSetCmdAA(true, 1, ePrivDrvCfgCmdTypeGet, ePrivDrvCmdRstMax, ePrivDrvParamAddr097, 0, NULL, 0);
    if(bRst)
    {
        bRst = sPrivDrvGetCmd0A(false, &pCmd0A);
        if((bRst) && (pCmd0A->eAddr == ePrivDrvParamAddr097))
        {
            ...
            bRst = sPrivDrvSetCmdAA(true, 1, ePrivDrvCfgCmdTypeSet, ePrivDrvCmdRstMax, ePrivDrvParamAddr097, sizeof(stParam1), (const u8 *)&stParam1, 0);
        }
        else
        {
            bRst = false;
...
bool sPrivDrvShellGetCpSn(const stShellPkt_t *pkg)
{
    ...
    bRst = sPrivDrvSetCmdAA(true, (u8)i32GunId, ePrivDrvCfgCmdTypeGet, ePrivDrvCmdRstMax, ePrivDrvParamAddr001, 0, NULL, 0);
    if(bRst == true)
    {
        sPrivDrvGetCmd0A(false, &pCmd0A);
        memset(u8Sn, 0, sizeof(u8Sn));
...
* param[in]         :     pkg      shell输入结构体
* return            :     bRst
* notice            :     查询桩时间:cpparam 0 2 0 1 2
                          设置桩时间:cpparam 0 2 1 1 2 180203040506
...
bool sPrivDrvShellCpParam(const stShellPkt_t *pkg)
{
    ...
    bRst = sPrivDrvSetCmdAA(i32Rpt, (u8)i32GunId, i32Cmd, i32Rst, i32Addr, i32Len, pDat, 0);
