...
bool chgFwUpdateCharger(const char* filename)
{
    ...
    EN_SLOGD(TAG, "file head is valid!");
    
    //3.检测设备类型是否一致
    EN_SLOGD(TAG, "check dev type is match...");
    //if(gGWInfo.charger_code != gChgFwChargerInfo->file_header_info.dev_type)
    //{
    //    EN_SLOGE(TAG, "dev_type don't match dev:%02X file:%02X", gGWInfo.charger_code, gChgFwChargerInfo->file_header_info.dev_type);
    //    goto OUT;
    //}
    EN_SLOGD(TAG, "dev type is matched!");
