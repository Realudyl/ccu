...
//一些切换判断的阈值定义
#define cModAppAvgModUdcOpen            (180.0f)
...
typedef struct
{
    u8                                  u8Status;                               //当前开关状态
    u8                                  u8OpValue;                              //操作值
    u8                                  u8StatusEnd;                            //按该操作值执行切换后最终的稳态开关状态
    ePileGunIndex_t                     eGunIndexYl;                            //当前开关状态下Y接触器左侧(A枪侧)模块所属的枪号
    ePileGunIndex_t                     eGunIndexYr;                            //当前开关状态下Y接触器右侧(B枪侧)模块所属的枪号
    void                                (*pFunOp)(void);                        //按该操作值需要执行的切换动作函数指针
}stModAppAvgSwitchMatrix_t;

//u8OpValue 值计算公式
//eGunIndex:枪序号0/1, bFlag:开关---1/0
#define sModAppAvgCalcOpValue(eGunIndex, bFlag) ((1 << (eGunIndex * ePileGunIndexNum)) << bFlag)
...
extern bool sModAppAvgInit(i32 i32ModNum, eIo_t eIoAcContactor);
