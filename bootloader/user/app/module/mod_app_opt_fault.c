...
void sModAppOptModeFault_MDE(void)
{
    ...
    if(1)
    {
        ...
        {
            u32TimeStamp = xTaskGetTickCount();
            if(sFaultCodeChkCode(cPrivDrvGunIdChg, ePrivDrvFaultCode_dcShort) == true)
            {
                if(xTaskGetTickCount() - u32StopTimeStamp >= 5000)
                {
                    sFaultCodeSub(cPrivDrvGunIdChg, ePrivDrvFaultCode_dcShort);
                }
            }
        }
    }
