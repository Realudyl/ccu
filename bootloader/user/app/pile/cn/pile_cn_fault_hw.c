...
void sPileCnFaultCheck_OOCP(void)
{
    ...
    for(i = ePileGunIndex0; i < ePileGunIndexNum; i++)
    {
        ...
        if((abs(((u16)f32MeterIdc) - (u16BclReqChgCur/10)) >= (u16BclReqChgCur/100)) && (((u16)f32MeterIdc) > (u16BclReqChgCur/10)) && (sPileCnStateGet(i) == ePrivDrvGunWorkStatusCharging))
        {
...
void sPileCnFaultCheck_CDE(void)
{
    ...
    for(i = ePileGunIndex0; i < ePileGunIndexNum; i++)
    {
        ...
        if((u16ReqVolt == 0) && (u16ReqCurr == 0) && ((f32MeterVolt >= cSafeVolt) || (f32MeterCur >= cSafeCur)))
        {
