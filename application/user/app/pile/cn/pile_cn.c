...
void sPileCnProtCnsDispose(void);
...
/***************************************************************************************************
* Description                           :   认证时序开关相关的处理
* Author                                :   HH
* Creat Date                            :   2024-05-30
* notice                                :   底层驱动层需要根据认证时序开关去处理对应的事宜，对应的
处理都放在此处
****************************************************************************************************/
void sPileCnProtCnsDispose(void)
{
    //ADC drv层需要根据认证时序开关去更新采样周期，从而达到认证测试需要的时序
    sAdcDrvSetCycleCc(sEepromAppGetBlockFactoryProtocolFlag());
}
