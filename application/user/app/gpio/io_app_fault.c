...
void sIoAppFaultCheck_DCX(void);
void sIoAppFaultCheck_DCY(void);
void sIoAppFaultCheck_DCZ(void);
...
void sIoAppFaultCheck_DCF1(void)
{
    ...
    for(i = ePileGunIndex0; i < ePileGunIndexNum; i++)
    {
        ...
        if(((sIoDrvGetOutput(sIoGetDrK(i)) == SET) && (sAdcAppSampleGetFbStateK(i) == eAdcAppFbState11)) || ((sIoDrvGetOutput(sIoGetDrK(i)) == RESET) && (sAdcAppSampleGetFbStateK(i) == eAdcAppFbState00)))
        {
            ...
            {
                if(xTaskGetTickCount() - u32StopTimeStamp[i] >= 10000)
                {
...
void sIoAppFaultCheck_DCX(void)
{
    ...
    if((sIoDrvGetOutput(eIo_DR_X) == SET) && (sAdcAppSampleGetFbState(eAdcPin_FB_X) != eAdcAppFbState11))
    {
        if(xTaskGetTickCount() - u32StartTimeStamp >= cDcContactFaultDetectTime)
        {
            if(sFaultCodeChkCode(cPrivDrvGunIdChg, ePrivDrvFaultCode_ccuXKmErr) == false)
            {
                u32StopTimeStamp = xTaskGetTickCount();
                sFaultCodeAdd(cPrivDrvGunIdChg, ePrivDrvFaultCode_ccuXKmErr);
            }
        }
    }
    else if((sIoDrvGetOutput(eIo_DR_X) == RESET) && (sAdcAppSampleGetFbState(eAdcPin_FB_X) != eAdcAppFbState00))
    {
        if(xTaskGetTickCount() - u32StartTimeStamp >= cDcContactFaultDetectTime)
        {
            if(sFaultCodeChkCode(cPrivDrvGunIdChg, ePrivDrvFaultCode_ccuXKmErr) == false)
    ...
    if(((sIoDrvGetOutput(eIo_DR_X) == SET) && (sAdcAppSampleGetFbState(eAdcPin_FB_X) == eAdcAppFbState11)) || ((sIoDrvGetOutput(eIo_DR_X) == RESET) && (sAdcAppSampleGetFbState(eAdcPin_FB_X) == eAdcAppFbState00)))
    {
        u32StartTimeStamp = xTaskGetTickCount();
        if(sFaultCodeChkCode(cPrivDrvGunIdChg, ePrivDrvFaultCode_ccuXKmErr) == true)
        {
            if(xTaskGetTickCount() - u32StopTimeStamp >= 10000)
            {
                sFaultCodeSub(cPrivDrvGunIdChg, ePrivDrvFaultCode_ccuXKmErr);
            }
        }
    }
}
...
void sIoAppFaultCheck_DCY(void)
{
    ...
    if((sIoDrvGetOutput(eIo_DR_Y) == SET) && (sAdcAppSampleGetFbState(eAdcPin_FB_Y) != eAdcAppFbState11))
    {
        if(xTaskGetTickCount() - u32StartTimeStamp >= cDcContactFaultDetectTime)
        {
            if(sFaultCodeChkCode(cPrivDrvGunIdChg, ePrivDrvFaultCode_ccuYKmErr) == false)
            {
                u32StopTimeStamp = xTaskGetTickCount();
                sFaultCodeAdd(cPrivDrvGunIdChg, ePrivDrvFaultCode_ccuYKmErr);
            }
        }
    }
    else if((sIoDrvGetOutput(eIo_DR_Y) == RESET) && (sAdcAppSampleGetFbState(eAdcPin_FB_Y) != eAdcAppFbState00))
    {
        if(xTaskGetTickCount() - u32StartTimeStamp >= cDcContactFaultDetectTime)
        {
            if(sFaultCodeChkCode(cPrivDrvGunIdChg, ePrivDrvFaultCode_ccuYKmErr) == false)
    ...
    if(((sIoDrvGetOutput(eIo_DR_Y) == SET) && (sAdcAppSampleGetFbState(eAdcPin_FB_Y) == eAdcAppFbState11)) || ((sIoDrvGetOutput(eIo_DR_Y) == RESET) && (sAdcAppSampleGetFbState(eAdcPin_FB_Y) == eAdcAppFbState00)))
    {
        u32StartTimeStamp = xTaskGetTickCount();
        if(sFaultCodeChkCode(cPrivDrvGunIdChg, ePrivDrvFaultCode_ccuYKmErr) == true)
        {
            if(xTaskGetTickCount() - u32StopTimeStamp >= 10000)
            {
                sFaultCodeSub(cPrivDrvGunIdChg, ePrivDrvFaultCode_ccuYKmErr);
            }
        }
    }
}
...
void sIoAppFaultCheck_DCZ(void)
{
    ...
    if((sIoDrvGetOutput(eIo_DR_Z) == SET) && (sAdcAppSampleGetFbState(eAdcPin_FB_Z) != eAdcAppFbState11))
    {
        if(xTaskGetTickCount() - u32StartTimeStamp >= cDcContactFaultDetectTime)
        {
            if(sFaultCodeChkCode(cPrivDrvGunIdChg, ePrivDrvFaultCode_ccuZKmErr) == false)
            {
                u32StopTimeStamp = xTaskGetTickCount();
                sFaultCodeAdd(cPrivDrvGunIdChg, ePrivDrvFaultCode_ccuZKmErr);
            }
        }
    }
    else if((sIoDrvGetOutput(eIo_DR_Z) == RESET) && (sAdcAppSampleGetFbState(eAdcPin_FB_Z) != eAdcAppFbState00))
    {
        if(xTaskGetTickCount() - u32StartTimeStamp >= cDcContactFaultDetectTime)
        {
            if(sFaultCodeChkCode(cPrivDrvGunIdChg, ePrivDrvFaultCode_ccuZKmErr) == false)
    ...
    if(((sIoDrvGetOutput(eIo_DR_Z) == SET) && (sAdcAppSampleGetFbState(eAdcPin_FB_Z) == eAdcAppFbState11)) || ((sIoDrvGetOutput(eIo_DR_Z) == RESET) && (sAdcAppSampleGetFbState(eAdcPin_FB_Z) == eAdcAppFbState00)))
    {
        u32StartTimeStamp = xTaskGetTickCount();
        if(sFaultCodeChkCode(cPrivDrvGunIdChg, ePrivDrvFaultCode_ccuZKmErr) == true)
        {
            if(xTaskGetTickCount() - u32StopTimeStamp >= 10000)
            {
                sFaultCodeSub(cPrivDrvGunIdChg, ePrivDrvFaultCode_ccuZKmErr);
            }
        }
    }
}
