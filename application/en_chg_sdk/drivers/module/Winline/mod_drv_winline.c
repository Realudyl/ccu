...
void  sModDrvWinlineSetLowPowerSts(bool bSts);
...
bool sModDrvWinlineInit(i32 i32Port, i32 i32Baudrate)
{
    ...
    pModDrvWinlineCache->bLowPowerSts = true;                                   //默认AC交流接触器断开的 初值要给低功耗状态
...
/***************************************************************************************************
* Description                           :   永联科技 NXR 系列充电模块电源驱动 低功耗状态设定
* Author                                :   Hall
* Creat Date                            :   2024-12-18
* notice                                :   低功耗状态：外部会切断模块的AC交流接触器，模块断电
*                                           这个状态下，程序不要轮询数据
****************************************************************************************************/
void  sModDrvWinlineSetLowPowerSts(bool bSts)
{
    if(pModDrvWinlineCache != NULL)
    {
        pModDrvWinlineCache->bLowPowerSts = bSts;
    }
}
...
void sModDrvWinlineRecvTask(void *pParam)
{
    ...
    while(1)
    {
        while(sModDrvWinlineRecv() == true)
        {
            i32TimeoutCnt = 0;
            //防止一直处理在循环中出不来,10次后跳出
            i32RecvCnt++;
            if(i32RecvCnt > cModDrvWinlineRecvTaskCycle)//cModDrvWinlineRecvTaskCycle / 1 (125kbps下每帧报文传输时间1ms)
            {
                i32RecvCnt = 0;
                break;
            }
        ...
        if(pModDrvWinlineCache->bLowPowerSts == true)
        {
            //低功耗状态下不打印
            i32TimeoutCnt = 0;
        }
...
void sModDrvWinlineSendTask(void *pParam)
{
    ...
    while(1)
    {
        if(pModDrvWinlineCache->bLowPowerSts == false)
        {
            //非低功耗状态下才轮询数据
...
bool sModDrvWinlineReadData(void)
{
    ...
    static stModDrvWinlineRegMap_t stRegMap[8] =
    {
        {eModDrvWinlineRegGetUdcOut,        1,      0},
        {eModDrvWinlineRegGetIdcOut,        1,      0},
        {eModDrvWinlineRegGetTenv,          5,      0},
        {eModDrvWinlineRegGetUa,            5,      0},
        {eModDrvWinlineRegGetUb,            5,      0},
        {eModDrvWinlineRegGetUc,            5,      0},
        {eModDrvWinlineRegGetWarningCode,   5,      0},
        {eModDrvWinlineRegGetFanS,          5,      0},
    ...
    for(m = 0; m < sArraySize(stRegMap); m++)
    {
        stRegMap[m].u8Cnt++;
        if(stRegMap[m].u8Cnt < stRegMap[m].u8Cycle)
        {
            continue;
        }
        else
        ...
        for(i = cModDrvWinlineAddrMin; i < cModDrvWinlineAddrNumMax; i++)
        {
            ...
            {
                bRst = bRst & sModDrvWinlineSend(i, eModDrvWinlineFunGet, stRegMap[m].eReg, unPayload, true);
            }
        }
