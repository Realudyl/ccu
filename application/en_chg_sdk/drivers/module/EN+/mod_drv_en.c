...
void  sModDrvEnSetLowPowerSts(bool bSts);
...
bool sModDrvEnInit(i32 i32Port, i32 i32Baudrate)
{
    ...
    pModDrvEnCache->bLowPowerSts = true;                                        //默认AC交流接触器断开的 初值要给低功耗状态
...
void  sModDrvEnSetLowPowerSts(bool bSts)
{
    if(pModDrvEnCache != NULL)
    {
        pModDrvEnCache->bLowPowerSts = bSts;
    }
}
...
void sModDrvEnRecvTask(void *pParam)
{
    ...
    while(1)
    {
        while(sModDrvEnRecv() == true)
        {
            i32TimeoutCnt = 0;
            //防止一直处理在循环中出不来,10次后跳出
            i32RecvCnt++;
            if(i32RecvCnt > cModDrvEnRecvTaskCycle)//cModDrvEnRecvTaskCycle / 1 (125kbps下每帧报文传输时间1ms)
            {
                i32RecvCnt = 0;
                break;
            }
        ...
        if(pModDrvEnCache->bLowPowerSts == true)
        {
            //低功耗状态下不打印
            i32TimeoutCnt = 0;
        }
...
void sModDrvEnSendTask(void *pParam)
{
    ...
    while(1)
    {
        if(pModDrvEnCache->bLowPowerSts == false)
        {
            //非低功耗状态下才轮询数据
