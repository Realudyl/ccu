...
#if(cPrivDrv08RecvPrintf == 1)
    EN_SLOGW(TAG,"Id:%d,充电时长:%u,开始电表:%u,当前电表:%u,累计金额:%u",
    pCmd08->u8GunId,
    pData->stGun[u8Index].stRtData.u32ChargingTime,
    pData->stGun[u8Index].stRtData.u32MeterStart,
    pData->stGun[u8Index].stRtData.u32MeterNow,
    pData->stGun[u8Index].stRtData.u32UsedMoney);
#endif
