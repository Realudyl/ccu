...
bool sPrivDrvPktRecv03(const u8 *pBuf, i32 i32Len)
{
    ...
    if(bRst == true)
    {
        ...
        else
        {
            ...
            //故障和告警位需要同步到桩数据
            memcpy(&pData->stChg.stCache03.unFaultByte,   &pCmd03->unFaultByte,   sizeof(pCmd03->unFaultByte));
            memcpy(&pData->stChg.stCache03.unWarningByte, &pCmd03->unWarningByte, sizeof(pCmd03->unWarningByte));
            ...
            //空闲/插枪BMS数据清除
            if((pData->stGun[u8Index].eWorkStatus < ePrivDrvGunWorkStatusCharging) && (pData->stGun[u8Index].bBmsDataUpdata))
            {
                pData->stGun[u8Index].bBmsDataUpdata = false;
                xSemaphoreTake(pPrivDrvCache->unData.stMaster.stGun[u8Index].hCache13Mutex, 0);
                memset(&pData->stGun[u8Index].stBmsData, 0, sizeof(stPrivDrvCmd13_t));
                EN_SLOGW(TAG, "BMS缓存清除");
            }
        }
    }
...
#if(cPrivDrv03RecvPrintf == 1)
    EN_SLOGE(TAG, "Id:%d,枪连接状态:%d,当前工作状态:%d,上一工作状态:%d", pCmd03->u8GunId,
    pCmd03->eGunConnStatus, pCmd03->eGunWorkStatus, pCmd03->eGunWorkStatusOld);
