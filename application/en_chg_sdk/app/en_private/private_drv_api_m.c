...
/**********************************************************************************************
* Description       :     可读数据 get:获取桩状态
* Author            :     XRG
* modified Date     :     2024-12-19
* notice            :     用于检查桩端桩状态
***********************************************************************************************/
bool sPrivDrvGetStatus(ePrivDrvStatus_t *pSts)
{
    char u8Ver[cPrivDrvLenSwVer + 2];
    
    if(pPrivDrvCache != NULL)
    {
        if(pPrivDrvCache->unData.stMaster.stChg.bCache02Flag == false)
        {
            *pSts = ePrivDrvStatusNoLogin;
        }
        else
        {
            memset(&u8Ver, 0, sizeof(u8Ver));
            sPrivDrvGetSwVer(sizeof(u8Ver), (u8 *)u8Ver);
            if(strcmp(u8Ver, cChgSwVerBootloader) == 0)
            {
                *pSts = ePrivDrvStatusBoot;
            }
            else
            {
                *pSts = ePrivDrvStatusFinish;
            }
        }
...
/**********************************************************************************************
* Description       :     可读可写数据 get:打印类型
* Author            :     XRG
* modified Date     :     2024-12-13
* notice            :     对内网打印减少部分数据,减少日志负担
***********************************************************************************************/
bool sPrivDrvGetPrintfType(void)
{
    if(pPrivDrvCache != NULL)
    {
        return(pPrivDrvCache->bPrintfType);
    }
...
* Author            :     Hall
* modified Date     :     2023-10-21
* notice            :     注意枪号 1~cPrivDrvGunNumMax:表示枪号
***********************************************************************************************/
...
***********************************************************************************************/
bool sPrivDrvGetCmdParam(bool bRptFlag, stPrivDrvCmd0A_t **pCmd0A)
{
    bool bRst;
    ePrivDrvStatus_t  eSts = ePrivDrvStatusMax;
    
    sPrivDrvGetStatus(&eSts);
    if(eSts != ePrivDrvStatusFinish)
    {
        EN_SLOGE(TAG, "桩端处于%d !!!", eSts);
        return(false);
    }
...
/**********************************************************************************************
* Description       :     可读数据 get:充电充电功率
* Author            :     XRG
* modified Date     :     2024-12-19
* notice            :     功率与cmd08一致
***********************************************************************************************/
bool sPrivDrvGetChargPower(u8 u8GunId, u32 *pPower)
{
    if((pPower != NULL) && (pPrivDrvCache != NULL) && (u8GunId >= cPrivDrvGunIdBase) && (u8GunId <= cPrivDrvGunNumMax))
    {
        (*pPower) = pPrivDrvCache->unData.stMaster.stGun[u8GunId - cPrivDrvGunIdBase].stCache0E.u32Power;
        return(true);
    }
...
/**********************************************************************************************
* Description       :     可读数据 get:当前电表值
* Author            :     XRG
* modified Date     :     2024-12-19
* notice            :     实时电表值
***********************************************************************************************/
bool sPrivDrvGetMeterNow(u8 u8GunId, u32 *pMeter)
{
    if((pMeter != NULL) && (pPrivDrvCache != NULL) && (u8GunId >= cPrivDrvGunIdBase) && (u8GunId <= cPrivDrvGunNumMax))
    {
        (*pMeter) = pPrivDrvCache->unData.stMaster.stGun[u8GunId - cPrivDrvGunIdBase].stCache0E.u32MeterNow;
        return(true);
    }
...
* notice            :     
***********************************************************************************************/
bool sPrivDrvGetBmsRpt(u8 u8GunId, bool bMutex, stPrivDrvCmd13_t **pBmsData)
{
    bool bRst;
    ...
    if((u8GunId >= cPrivDrvGunIdBase) && (u8GunId <= cPrivDrvGunNumMax))
    {
        if(pPrivDrvCache != NULL)
        {
            ...
            bRst = pPrivDrvCache->unData.stMaster.stGun[u8GunId - cPrivDrvGunIdBase].bBmsDataUpdata;
            if(bMutex)
            {
...
/**********************************************************************************************
* Description       :     可读可写数据 set:设置打印类型
* Author            :     XRG
* modified Date     :     2024-12-13
* notice            :     对内网打印减少部分数据,减少日志负担
***********************************************************************************************/
bool sPrivDrvSetPrintfType(bool bTpye)
{
    if(pPrivDrvCache != NULL)
    {
        pPrivDrvCache->bPrintfType = bTpye;
        return(true);
    }
...
bool sPrivDrvSetCmdParam(bool bRptFlag, u8 u8GunId, ePrivDrvCfgCmdType_t eType, ePrivDrvCmdRst_t eRst, ePrivDrvParamAddr_t eAddr, u16 u16ParamLen, const u8 *pParam, u8 u8RepeatCnt)
{
    ...
    if(!((eAddr != ePrivDrvParamAddr018) || (eAddr != ePrivDrvParamAddr001)))
    {
        sPrivDrvGetStatus(&eSts);
        if(eSts != ePrivDrvStatusFinish)
        {
