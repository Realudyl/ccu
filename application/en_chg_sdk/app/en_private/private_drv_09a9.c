...
bool sPrivDrvPktSendA9(u8 u8GunId, ePrivDrvCmdRst_t eRst, i32 i32OrderIdLen, const u8 *pOrderId, i32 i32SrvTradeIdLen, const u8 *pSrvTradeId)
{
    ...
    pCmdA9->eRst                = eRst;
    if((pOrderId != NULL) && (i32OrderIdLen > 0))
    {
        memcpy(pCmdA9->u8OrderId,     pOrderId,    i32OrderIdLen);
    }
    if((pSrvTradeId != NULL) && (i32SrvTradeIdLen > 0))
    {
...
#if(cPrivDrv09RecvPrintf == 1)
    EN_SLOGW(TAG,"Id:%d,充电时长:%u,开始电表:%u,当前电表:%u,累计金额:%u,开始时间:%s,结束时间:%s,原因:%d",
    pCmd09->u8GunId,
    pData->stGun[u8Index].stRcdData.u32ChargingTime,
    pData->stGun[u8Index].stRcdData.u32MeterStart,
    pData->stGun[u8Index].stRcdData.u32MeterStop,
    pData->stGun[u8Index].stRcdData.u32UsedMoney,
    sPrivDrvTimeToStr(&pData->stGun[u8Index].stRcdData.stTimeStart),
    sPrivDrvTimeToStr(&pData->stGun[u8Index].stRcdData.stTimeStop),
    pData->stGun[u8Index].stRcdData.eStopReason);
#endif
