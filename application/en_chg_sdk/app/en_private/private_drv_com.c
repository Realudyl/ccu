...
#if(cPrivDrvRxScanEn == 1)
    esp_err_t err = uart_driver_install(cPrivDrvComUartNum, cPrivDrvComRxBuffSize, 0, 0, NULL, 0);
#else
...
i32 sPrivComRecvEsp32(u32 u32Port, u32 u32Len, u8 *u8RxBuf)
{
    i32 i32ReadedLen = uart_read_bytes(u32Port, u8RxBuf, u32Len, pdMS_TO_TICKS(300));
...
i32 sPrivComRecvEsp32(u32 u32Port, u32 u32Len, u8 *u8RxBuf)
{
    ...
    if(xQueueReceive(pPrivDrvCache->hUartQueue, (void * )&event, (pdMS_TO_TICKS(300))))
    {
        switch (event.type) 
        {
        case UART_DATA:
