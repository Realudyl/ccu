...
#define cChgSwVerBootDc                 "boot"                                  //直流脚本boot模式
...
typedef enum
{
    ...
    ePrivDrvCmd11                       = 0x11,                                 //安全事件通知
    ePrivDrvCmdB1                       = 0xB1,                                 //安全事件通知应答
...
//BMS转换
//前期BMS旧状态
#ifndef cSdkPrivDrvBmsOldStsEn
#define cPrivDrvBmsOldStsEn             (0)
#else
#define cPrivDrvBmsOldStsEn             (cSdkPrivDrvBmsOldStsEn)
#endif


typedef enum
{
    ePrivDrvStatusNull                  = 0,                                    //未建立通信
    ePrivDrvStatusNoLogin,                                                      //未收到签到
    ePrivDrvStatusBoot,                                                         //处于boot
    ePrivDrvStatusFinish,                                                       //已完成--正常

    ePrivDrvStatusMax
}__attribute__((packed)) ePrivDrvStatus_t;
...
#if(cPrivDrvBmsOldStsEn == 1)
    ePrivDrvGunWorkStatusStarting       = 0x0a,                                 //无                  BMS启动中(直流)
    ePrivDrvGunWorkStatusBmsPreparing   = 0x0b,                                 //无                  BMS开机准备中(直流)
    ePrivDrvGunWorkStatusBmsStop        = 0x0c,                                 //无                  BMS运行暂停(直流)
#endif    
...
typedef struct
{
    ...
    //Byte[2]
    u8                                  bPhaseInver         : 1;                //输入反接
    u8                                  bIleakHw            : 1;                //漏电流电路异常
    u8                                  bRsvd21             : 1;                //预留
    u8                                  bElectErr           : 1;                //电量统计异常
    u8                                  bLoadMeter          : 1;                //负载平衡电表异常
    u8                                  bCtAbnormal         : 1;                //CT环采集异常
    u8                                  bMeterCommFault     : 1;                //电表通讯故障
    u8                                  bUcpAbnormal        : 1;                //CP电压异常
    
    //Byte[3]
    u8                                  bGunTempErr         : 1;                //枪温度异常
    u8                                  bEnviroTempErr      : 1;                //环境温度异常
    u8                                  bRsvd31             : 1;                //预留
    u8                                  bRsvd32             : 1;                //预留
    u8                                  bRsvd33             : 1;                //预留
    u8                                  bGunLockErr         : 1;                //枪锁异常
    u8                                  bGunCurrentErr      : 1;                //枪电流异常
...
{
    //Byte[0]
    u8                                  bParkingObstacle    : 1;                //车位有障碍物
    u8                                  bParkingLowVolt     : 1;                //车位锁欠压
    u8                                  bGunCc              : 1;                //充电枪cc检测
    u8                                  bGunCp              : 1;                //充电枪cp检测
    u8                                  bLoadReudction      : 1;                //降载
    u8                                  bInsulationResErr   : 1;                //绝缘阻抗异常
    u8                                  bHumidityOverMax    : 1;                //湿度过大
    u8                                  bIsoOverTime        : 1;                //ISO 检测超时
    
    //Byte[1]
    u8                                  bEleLockFeedbackErr : 1;                //电子锁反馈异常
    u8                                  bChgVoltMismatch    : 1;                //充电电压与桩不匹配
    u8                                  bModelErr           : 1;                //充模块异常
    u8                                  bGunTempErr         : 1;                //枪温度异常
    u8                                  bModelNumberErr     : 1;                //模块数量异常
    u8                                  bFanErr             : 1;                //风扇异常
    u8                                  bRsv1               : 1;                //预留
    u8                                  bRsv2               : 1;                //预留
...
//充电参数---针对不同充电模式 单位不一样
 #define cPrivDrvChargingParamErr       0xFFFFFFFF                              //异常订单
typedef union
{
...
typedef enum
{
    ...
    ePrivDrvCardAuthFailRsn18           = 18,                                   //该账户不能在此桩上充电
    ePrivDrvCardAuthFailRsn19           = 19,                                   //电站电容不足
    ePrivDrvCardAuthFailRsn20           = 20,                                   //该桩存在未结账记录
