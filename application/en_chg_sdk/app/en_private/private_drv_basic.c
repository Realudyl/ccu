...
void sPrivDrvDataPrint(bool bSendFlag, const u8 *pBuf, u16 u16Len)
{
    ...
    u16SendLen = u16Len;
    if((bSendFlag)
    && (u16Len       >= cPrivDrvDataPrintMax)
    && ((pHead->eCmd == ePrivDrvCmdAB)
    || (pHead->eCmd  == ePrivDrvCmdAC)))
    {
    ...
    snprintf(u8HeadStr, sizeof(u8HeadStr), "内网%s(Cmd:%02X Len:%03d)", u8SendRecvStr[bSendFlag], pHead->eCmd, u16Len);
...
/**********************************************************************************************
* Description       :     串口数据 标准时间转字符串
* Author            :     XRG
* modified Date     :     2024-08-16
* notice            :     
***********************************************************************************************/
char *sPrivDrvTimeToStr(void *pDat)
{
    ...
    stTime_t *pTime = (stTime_t *)pDat;
    memset(pBuff, 0, sizeof(pBuff));
    snprintf(pBuff, sizeof(pBuff),"%02d-%02d-%02d %02d:%02d:%02d", pTime->year,
    pTime->mon,pTime->day,pTime->hour,pTime->min,pTime->sec);
